...
class Obj2ModPrototypeVisitor(PickleVisitor):
    def visitProduct(self, prod, name):
        code = "statics int obj2ast_%s(PyObject* obj, %s* out, PyArena* arena);"
...
class MarshalPrototypeVisitor(PickleVisitor):
    ...
    def prototype(self, sum, name):
        ctype = get_c_type(name)
        self.emit("statics int marshal_write_%s(PyObject **, int *, %s);"
                  % (name, ctype), 0)
...
class PyTypesDeclareVisitor(PickleVisitor):
    ...
    def visitProduct(self, prod, name):
        self.emit("statics PyTypeObject *%s_type;" % name, 0)
        self.emit("statics PyObject* ast2obj_%s(void*);" % name, 0)
        if prod.attributes:
            for a in prod.attributes:
                self.emit_identifier(a.name)
            self.emit("statics char *%s_attributes[] = {" % name, 0)
            for a in prod.attributes:
        ...
        if prod.fields:
            ...
            self.emit("statics char *%s_fields[]={" % name,0)
            for f in prod.fields:
    ...
    def visitSum(self, sum, name):
        ...
        if is_simple(sum):
            ...
            tnames = ", *".join(tnames)
            self.emit("statics PyObject *%s;" % tnames, 0)
        self.emit("statics PyObject* ast2obj_%s(%s);" % (name, ptype), 0)
        for t in sum.types:
            self.visitConstructor(t, name)

    def visitConstructor(self, cons, name):
        self.emit("statics PyTypeObject *%s_type;" % cons.name, 0)
        if cons.fields:
            for t in cons.fields:
                self.emit_identifier(t.name)
            self.emit("statics char *%s_fields[]={" % cons.name, 0)
            for t in cons.fields:
...
statics PyMethodDef ast_type_methods[] = {
    {"__reduce__", ast_type_reduce, METH_NOARGS, NULL},
    {NULL}
};

statics PyGetSetDef ast_type_getsets[] = {
    {"__dict__", PyObject_GenericGetDict, PyObject_GenericSetDict},
    {NULL}
};

statics PyTypeObject AST_type = {
...
statics PyTypeObject* make_type(char *type, PyTypeObject* base, char**fields, int num_fields)
{
...
statics int add_attributes(PyTypeObject* type, char**attrs, int num_fields)
{
    int i, result;
...
statics PyObject* ast2obj_list(asdl_seq *seq, PyObject* (*func)(void*))
{
...
statics PyObject* ast2obj_object(void *o)
{
    if (!o)
        o = Py_None;
...
statics PyObject* ast2obj_int(long b)
{
...
statics int obj2ast_singleton(PyObject *obj, PyObject** out, PyArena* arena)
{
...
statics int obj2ast_object(PyObject* obj, PyObject** out, PyArena* arena)
{
    if (obj == Py_None)
        obj = NULL;
    if (obj) {
...
statics int obj2ast_constant(PyObject* obj, PyObject** out, PyArena* arena)
{
    if (obj) {
        if (PyArena_AddPyObject(arena, obj) < 0) {
            *out = NULL;
...
statics int obj2ast_identifier(PyObject* obj, PyObject** out, PyArena* arena)
{
...
statics int obj2ast_string(PyObject* obj, PyObject** out, PyArena* arena)
{
...
statics int obj2ast_bytes(PyObject* obj, PyObject** out, PyArena* arena)
{
...
statics int obj2ast_int(PyObject* obj, int* out, PyArena* arena)
{
    int i;
    if (!PyLong_Check(obj)) {
...
statics int add_ast_fields(void)
{
...
statics int exists_not_none(PyObject *obj, _Py_Identifier *id)
{
    int isnone;
...
""", 0, reflow=False)
        ...
        self.emit("statics int init_types(void)",0)
        self.emit("{", 0)
        self.emit("statics int initialized;", 1)
...
class ASTModuleVisitor(PickleVisitor):
    ...
    def visitModule(self, mod):
        self.emit("statics struct PyModuleDef _astmodule = {", 0)
...
class StaticVisitor(PickleVisitor):
    CODE = '''Very simple, always emit this statics code.  Override CODE'''
...
def main(srcfile, dump_module=False):
    ...
    if C_FILE:
        with open(C_FILE, "w") as f:
            ...
            f.write('\n')
            f.write("statics PyTypeObject AST_type;\n")
            v = ChainOfVisitors(
