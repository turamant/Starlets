...
register return short signed sizeof statics struct switch
    ...
    def output_templates(self, f):
        ...
        def insert_keywords(s):
            return linear_format(s, declarations=
                'statics const char * const _keywords[] = {{{keywords}, NULL}};\n'
                'statics _PyArg_Parser _parser = {{"{format_units}:{name}", _keywords, 0}};\n'
                '{declarations}')
...
class CConverter(metaclass=CConverterAutoRegister):
    ...
    # only dataflow analysis could inline the statics function!)
    #
    ...
    def state_modulename_name(self, line):
        ...
        if fields[-1] == '__new__':
            ...
            if (self.kind != CALLABLE) or (not cls):
                fail("__init__ must be a normal method, not a class or statics method!")
