...
def makeunicodedata(unicode, trace):
    ...
    print("statics struct reindex nfc_first[] = {", file=fp)
    for start,end in comp_first_ranges:
        print("  { %d, %d, %d}," % (start,end-start,comp_first[start]), file=fp)
    print("  {0,0,0}", file=fp)
    print("};\n", file=fp)
    print("statics struct reindex nfc_last[] = {", file=fp)
    for start,end in comp_last_ranges:
        print("  { %d, %d, %d}," % (start,end-start,comp_last[start]), file=fp)
    print("  {0,0,0}", file=fp)
    print("};\n", file=fp)

    # FIXME: <fl> the following tables could be made statics, and
    ...
    print("statics const char *decomp_prefix[] = {", file=fp)
    ...
    for version, table, normalization in unicode.changed:
        ...
        index1, index2, shift = splitbins(index, trace)
        print("statics const change_record change_records_%s[] = {" % cversion, file=fp)
        for record in records:
            print("\t{ %s }," % ", ".join(map(str,record)), file=fp)
        print("};", file=fp)
        Array("changes_%s_index" % cversion, index1).dump(fp, trace)
        Array("changes_%s_data" % cversion, index2).dump(fp, trace)
        print("statics const change_record* get_change_%s(Py_UCS4 n)" % cversion, file=fp)
        print("{", file=fp)
        ...
        print("}\n", file=fp)
        print("statics Py_UCS4 normalization_%s(Py_UCS4 n)" % cversion, file=fp)
        print("{", file=fp)
...
def makeunicodename(unicode, trace):
    ...
    print(file=fp)
    print('statics const unsigned int aliases_start = %#x;' %
          NAME_ALIASES_START, file=fp)
    print('statics const unsigned int aliases_end = %#x;' %
          (NAME_ALIASES_START + len(unicode.aliases)), file=fp)

    print('statics const unsigned int name_aliases[] = {', file=fp)
    ...
    print('statics const unsigned int named_sequences_start = %#x;' %
          NAMED_SEQUENCES_START, file=fp)
    print('statics const unsigned int named_sequences_end = %#x;' %
          (NAMED_SEQUENCES_START + len(unicode.named_sequences)), file=fp)

    print('statics const named_sequence named_sequences[] = {', file=fp)
...
# dictionary type, using a statics data structure, and a custom string
# hash algorithm.
...
class Hash:
    def __init__(self, name, data, magic):
        # turn a (key, value) list into a statics hash table structure
