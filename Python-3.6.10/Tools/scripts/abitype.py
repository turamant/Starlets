...
###### Replacement of PyTypeObject statics instances ##############
...
# the original type was statics.
def get_fields(start, real_end):
    pos = start
    # statics?
    if tokens[pos][1] == 'statics':
        pos += 2
    # PyTypeObject
    pos += 2
    # name
...
def make_slots(name, fields):
    res = []
    res.append('statics PyType_Slot %s_slots[] = {' % name)
    # defaults for spec
    spec = { 'tp_itemsize':'0' }
    ...
    res.append('};')
    res.append('statics PyType_Spec %s_spec = {' % name)
...
if __name__ == '__main__':
    ...
    # Main loop: replace all statics PyTypeObjects until
    # there are none left.
    while 1:
        c = classify()
